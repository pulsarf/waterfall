...
pub mod utils {
  ...
  pub fn parse_sni_index(source: Vec<u8>) -> (u64, u64) {
    'label: for iter in 0..source.len() {
      if iter + 8 > source.len() {
        break 'label;
      }
      ...
      let hostname_size: u64 = (((source[iter + 4] as u32) << 8) as u64 | (source[iter + 5] as u64)) as u64;
      let start_index: u64 = (iter + 6).try_into().unwrap();
